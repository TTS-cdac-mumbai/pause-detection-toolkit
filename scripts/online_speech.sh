...
#printf "( " $base_name >> $path/txt.done.data


###########only for first line###################################
first_wave_name=$(awk 'NR==1{print $1}' $path/output/wav_splitting/wav_list) 
test="part"
	cut_index1=$(awk -v a="$first_wave_name" -v b="$test" 'BEGIN{print index(a,b)}')
	cut_ind1=`expr $cut_index1 - 1`
	echo "cutting index is=$cut_ind1"
	basic_name1=$(echo $first_wave_name | cut -c1-$cut_ind1)
	echo "basicname1=$basic_name1"
	printf "atish $basic_name1" >> atti
	printf "( $basic_name1 ">> $path/txt.done.data
...
do
	...
	then
		...
		#printf " \" )"  >> .txt.done.data
               echo "prev from if and basic==$prev\t $basic_name" >>att
	...
	if [ "$basic_name" = "$prev" ] # prev name contains the current file name and basic also same
	then
		...
		printf " \")" >> txt.done.data 
		echo "Base name==$basic_name\n" >> att
		printf "( " $basic_name >> txt.done.data

	fi
done


 awk 'BEGIN {FS=OFS=","} NF--' txt.done.data > txt
 sed 's/$/ " )/g' txt > txt.done.data
...
bash /media/rachana/sda8/workspace-1nov18/pause_detection_toolkit/DNN_SEGMENTATION/SEGMENTATION/resources/text-correction/runJava.sh
